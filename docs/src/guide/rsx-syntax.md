...
Two things to know before reaching for it. The branch's `RenderScope` is
disposed on every hide, so effects *created inside the branch closure* stop; put
the reactive wiring in the same scope as the `panel` binding, not in the closure.
And a subtree that is out of the page is not laid out, so a measurement taken
while it is hidden is stale.

Plain markup in a branch — `if open.get() { p { "hi" } }` — is rebuilt on every
show, which is what you want for anything cheap.
...
On a positioned element the `top`/`right`/`bottom`/`left` shorthands accept any CSS length — `px`, `%`, `em`, `var()`, `calc()` — not just the spacing scale. A `calc()` mixing `%` with a length (`calc(50% - 10px)`) resolves correctly on desktop too (#278) — for insets, sizes, margins, padding, gaps, transforms and border radii alike. One named limitation: a non-affine `calc()` (a `min()`/`max()`/`clamp()` with a percentage inside) resolves as its large-container linearization on desktop rather than tracking the breakpoint.
...
Every author of an inline style — a `style:` prop, a style shorthand, a `set_style()` from a component or the runtime — splits and re-joins the attribute with the same parser ([issue #670](https://github.com/joeleaver/rinch/issues/670)). Four things follow that you can rely on:
...
- **A property name is ASCII case-insensitive**, as CSS makes it ([issue #711](https://github.com/joeleaver/rinch/issues/711)). `style: "color: blue; COLOR: red"` is one property declared twice, not two properties, and a later `set_style("color", …)` replaces it however either was spelled. A **custom property** is the exception and is compared exactly: `--Foo` and `--foo` are two variables, here and in a browser.
...
- **…but the value that survives is the `!important` one**, which is not always the last. `style: "color: red !important; color: blue"` stays red, as in a browser. The slot still moves to the last declaration's position; only the value is decided by priority.

One caveat on the last two, and it only bites on invalid CSS ([issue #722](https://github.com/joeleaver/rinch/issues/722)): rinch collapses duplicates **before** validity where a browser collapses after. A browser throws an invalid declaration away as it parses, so the duplicate never competes; rinch keeps it, and the property can end up with no usable value. `style: "color: notacolor !important; color: blue"` computes blue in a browser and falls back to black here. Two valid declarations — the only shape worth writing — behave identically.

What the attribute reads back as afterwards is the **CSSOM-touched** form: names lowercased, duplicates collapsed. A browser keeps the author's bytes in `getAttribute("style")` until something touches the CSSOM and rinch re-serialises on every merge, so rinch reaches that form one write earlier — the two agree on the result, not on when it appears. An element nobody has written to since is untouched either way.

`/* … */` comments are removed before any of this — from a property name as readily as a value — everywhere CSS sees one, and each leaves a space behind, because a comment ends the token before it even though it is not one itself. The two places CSS does not see a comment are inside a string and inside an unquoted `url(…)`, which is a single token: `url(http://a/*b*/c.png)` keeps its `/*b*/` and names the image the author wrote, as in a browser. There is one place a browser sees a comment and keeps it where rinch removes it — the interior of a **custom property** value, where Chrome reads `--x: 1px /* c */ 2px` back verbatim and rinch reads it back as `1px  2px`. The token stream is the same either way, so a `var()` substitution cannot tell; only the attribute string differs. An `!important` is carried through the value untouched; it is read, never rewritten, and the reading honours CSS's own looseness — `red!important`, `red ! IMPORTANT` and `red !ImPoRtAnT` are all important, while a `!important` inside a string or `url(…)`, or one whose `!` is escaped, is ordinary text.
