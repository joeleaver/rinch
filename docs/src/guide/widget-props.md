...
```rust
// Shorthands work on both HTML elements and widgets
...
**All props are reactive:** Every widget prop accepts a reactive closure `{|| expr}` in addition to a static value. When any prop uses a closure, the widget automatically re-renders when the signals inside change:
...
| `onclick` | `Option<WidgetCallback>` | `None` | Click handler |
...
| `onsubmit` | `Option<WidgetCallback>` | `None` | Fires on Enter key |
...
| `ontoggle` | `Option<WidgetCallback>` | `None` | Fires when visibility toggled |
| `oninput` | `Option<InputCallback>` | `None` | Receives `String` |
...
| `size` | `Option<String>` | `None` | |
| `radius` | `Option<String>` | `None` | |
| `onincrement` | `Option<WidgetCallback>` | `None` | |
| `ondecrement` | `Option<WidgetCallback>` | `None` | |
...
| `onchange` | `Option<WidgetCallback>` | `None` | |
...
| `icon` | `Option<Icon>` | `None` | |
| `onclose` | `Option<WidgetCallback>` | `None` | |
...
Sub-widgets: **PopoverTarget** (no props), **PopoverDropdown** (no props).
...
| `color` | `Option<String>` | `None` | |
| `disabled` | `bool` | `false` | |
| `onclick` | `Option<WidgetCallback>` | `None` | |
...
Sub-widgets: **HoverCardTarget** (no props), **HoverCardDropdown** (no props).
...
**AccordionControl:** `disabled: bool`, `icon: Option<Icon>`, `onclick: Option<WidgetCallback>`.
...
| `on_minimize` | `Option<WidgetCallback>` | `None` | |
| `on_maximize` | `Option<WidgetCallback>` | `None` | |
| `on_close` | `Option<WidgetCallback>` | `None` | |
...
|------|-----------|---------|
| `WidgetCallback` | `Rc<dyn Fn()>` | onclick, onclose, onsubmit, onchange (toggle) |
