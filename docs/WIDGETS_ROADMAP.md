...
1. Each widget should implement the `Widget` trait from `rinch-core`
2. Use CSS classes following the pattern `rinch-{component}--{modifier}`
3. Support theme CSS variables for colors, spacing, radius
4. Add styles to `styles.rs` via `generate_all_widget_styles()`
5. Export from `lib.rs`
6. Update documentation in `docs/src/guide/widgets.md`
