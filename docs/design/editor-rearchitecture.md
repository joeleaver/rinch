...
**Built — but the original "native contentEditable" sketch was REJECTED in favour of model-first reuse.** The recon proved `RinchDomEditorView` is already renderer-agnostic — it holds a `Weak<RefCell<dyn DomDocument>>` and mutates the host only through `rinch_core::dom::{DomDocument, NodeHandle}` + `rinch_editor_core` (unit-tested against `MockDomDocument`) — and `rinch-web`'s `WebDocument` already implements the full `DomDocument` trait incl. the editor's Range-API geometry queries. So the web editor is the **same** view + handle + overlays projecting onto the web DOM, with the container **not** `contenteditable`. One mental model with desktop, massive reuse, and (later) the same M9 collab adapter.

**Landed as two staged PRs:**
- **PR1 — extraction:** the renderer-agnostic editor (`view`/`handle`/`component`/`registry`/`styles`/caret-blink) moved out of the desktop-gated `rinch` crate into a new wasm-clean crate **`rinch-editor-view`**, which both `rinch` (desktop) and `rinch-web` depend on. `rinch`'s `editor` module became a re-export plus the desktop-only pieces (Parley block virtualization, AccessKit a11y, the `post_remote_delta` collab marshaller). Zero behaviour change.
- **PR2 — the web view:** (a) factored the pure-model caret motion + word/block selection into **`rinch-editor-core::motion`** (`CursorMotion`/`resolve_cursor_motion`/`word_range_at`/…), exposed on `EditorHandle` (`move_cursor`/`select_word_at`/`select_block_at`/`select_all`/`tab_cell`), so desktop and web share one implementation (only geometry-bound vertical motion + visual-line Home/End stay per-platform); (b) implemented `WebDocument::query_selection_rects` (Range `getClientRects`, block-local) and fixed `query_node_layout` to immediate-parent-relative (offset* double-counted nested blocks); (c) a web input-glue module (`rinch-web/src/editor_input.rs`): click-driven focus, `keydown`→command/move/insert, `caretRangeFromPoint`→`handle.pos_at`, vertical motion via goal-column hit-test, clipboard via `ClipboardEvent`, `setInterval` caret blink, a bubble-phase refresh so toolbar commands re-render the caret; (d) `examples/editor-web` (trunk).

**The two phases run synchronously on web** (after `update_dom` mutates the DOM, the first `getBoundingClientRect`/`getClientRects` forces a reflow so geometry is immediately available) — `editor_input` calls `update_all_carets` right after each edit.

- **Tests:** Playwright-driven in a real browser — type, format (toolbar + Ctrl+B/I/U), navigate (arrows/word/Home/End/vertical), select (click/double/triple/shift/drag), Ctrl+A (asserts the per-line `[data-pm-selection]` rects), Backspace; asserts the model-derived DOM (`data-pm-type`/`data-pm-mark`) and that caret/selection overlays land pixel-exactly on glyphs; `rinch-web` (no collab) links no rinch-dom/parley/automerge. (The `rinch` facade still drags `vello`/`wgpu` into the *dependency graph* on web — they are dead-code-eliminated from the bundle — a pre-existing facade feature-gating wart, not introduced here.)
- **v1 scope:** physical-key editing — all Latin text, shortcuts, navigation, selection, pixel-accurate overlays. **Clipboard and IME composition are a documented follow-up:** without `contenteditable` the browser dispatches no `paste`/`cut`/`compositionstart` events to a plain `<div>`, so both need a focused *editable* capture target (a hidden off-screen textarea) — one addition unblocks both and makes focus browser-native. Adversarial review (4 dimensions, every finding verified) caught the clipboard gap, a +1px overlay offset (the container border; fixed with a `DomDocument::content_origin_inset` accessor, default `(0,0)` so desktop is untouched), and edge cases in focus/drag/empty-block vertical motion — all fixed.
- **Closes:** #51 (the old CE engine it described was already removed in M8); proves the seam is genuinely renderer-agnostic.
