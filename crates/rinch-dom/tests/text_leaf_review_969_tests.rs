...
fn paint_at(doc: &mut RinchDocument, scale: f32) -> Vec<[u8; 4]> {
    ...
    rinch_dom::paint::paint_document(&doc.tree, &mut painter, scale as f64, VP, &mut doc.font_cx, &mut doc.layout_cx);
    painter.pixels().as_chunks::<4>().0.to_vec()
}

fn red(px: &[[u8; 4]]) -> usize {
    px.iter().filter(|p| p[3] > 0 && p[0] as i32 > p[1] as i32 + 80).count()
}
fn ink(px: &[[u8; 4]]) -> usize {
    px.iter().filter(|p| p[3] > 0 && (p[0] < 200 || p[1] < 200)).count()
}
...
fn cached_vs_fallback(css: &str, class: &str, label: &str, scale: f32) {
    ...
    let cached_lines = d.tree.get(t.0).unwrap().cached_text_parley.as_ref().map(|l| (l.len(), l.width(), l.height()));
    let a = paint_at(&mut d, scale);
    d.tree.nodes[t.0].cached_text_parley = None;
    let b = paint_at(&mut d, scale);
    let diff = a.iter().zip(&b).filter(|(x, y)| x != y).count();
    eprintln!("[{class} @{scale}] box {}x{} cached={cached_lines:?} ink cached={} fallback={} diffpx={diff}", lay.width, lay.height, ink(&a), ink(&b));
...
#[test] fn p3_plain_1() { cached_vs_fallback(CSS, "ic", "chip label text", 1.0); }
#[test] fn p3_plain_15() { cached_vs_fallback(CSS, "ic", "chip label text", 1.5); }
#[test] fn p3_plain_2() { cached_vs_fallback(CSS, "ic", "chip label text", 2.0); }
#[test] fn p2_narrow_1() { cached_vs_fallback(CSS, "ic narrow", "a long chip label text that wraps", 1.0); }
#[test] fn p2_narrow_15() { cached_vs_fallback(CSS, "ic narrow", "a long chip label text that wraps", 1.5); }
#[test] fn p2_width_1() { cached_vs_fallback(CSS, "ic w", "a long chip label text that wraps", 1.0); }
#[test] fn p2_ellipsis_1() { cached_vs_fallback(CSS, "ic ell", "a long chip label text that wraps", 1.0); }
#[test] fn p2_ls_1() { cached_vs_fallback(CSS, "ic ls", "chip label text", 1.0); }
#[test] fn p2_it_1() { cached_vs_fallback(CSS, "ic it", "chip label text", 1.0); }
...
#[test] fn p1_theme_inline_flex() { theme_case("inline-flex"); }
#[test] fn p1_theme_flex() { theme_case("flex"); }
#[test] fn p1_theme_inline_grid() { theme_case("inline-grid"); }
...
fn p5_move_label() {
    ...
    let build = |d: &mut RinchDocument, in_b: bool| {
        ...
        d.load_css(css);
        let body = d.body();
        let a = d.create_element("span"); d.set_attribute(a, "class", "a");
        let b = d.create_element("span"); d.set_attribute(b, "class", "b");
        let ta = d.create_text("x"); d.append_child(a, ta);
        let tb = d.create_text("y"); d.append_child(b, tb);
        let t = d.create_text("moved label");
        if in_b { d.append_child(b, t) } else { d.append_child(a, t) }
        d.append_child(body, a); d.append_child(body, b);
        (b, t)
    };
    let mut d = RinchDocument::new();
...
fn p6_display_flip() {
    ...
    settle(&mut d); let _ = paint_at(&mut d, 1.0);
    d.set_attribute(c, "class", "ic blk"); d.resolve_layout(VP.0, VP.1); let _ = paint_at(&mut d, 1.0);
    d.set_attribute(c, "class", "ic blk hot"); d.resolve_layout(VP.0, VP.1); let _ = paint_at(&mut d, 1.0);
    d.set_attribute(c, "class", "ic hot"); d.resolve_layout(VP.0, VP.1);
    let inc = paint_at(&mut d, 1.0);
    let mut f = RinchDocument::new();
...
fn p10_mid_transition_frame() {
    ...
    settle(&mut d); let _ = paint_at(&mut d, 1.0);
    d.set_attribute(c, "class", "ic hot"); d.resolve_layout(VP.0, VP.1); let _ = paint_at(&mut d, 1.0);
    for tr in d.tree.active_transitions.get_mut(&c.0).unwrap().values_mut() { tr.start_time_ms -= 500.0; }
    d.tick_transitions();
    d.resolve_layout(VP.0, VP.1);
    let brush = d.tree.get(t.0).unwrap().cached_text_parley.as_ref().map(|l| {
        let mut v = vec![];
        for line in l.lines() { for it in line.items() { if let parley::layout::PositionedLayoutItem::GlyphRun(g) = it { v.push(format!("{:?}", g.style().brush)); } } }
        v
    });
...
fn bench_doc(disp: &str, n: usize, trans: bool) -> (RinchDocument, Vec<NodeId>) {
    let mut d = RinchDocument::new();
    d.load_css(BASE_CSS);
    let t = if trans { "transition: color 150ms linear;" } else { "" };
...
fn hover_bench(disp: &str) {
    ...
    eprintln!("[bench hover {disp}] best {best:.1}us computes/200={}", d.tree.taffy_computes - c0);
}
#[test] #[ignore] fn bench_hover_flex() { hover_bench("flex"); }
#[test] #[ignore] fn bench_hover_iflex() { hover_bench("inline-flex"); }
#[test] #[ignore] fn bench_hover_block() { hover_bench("block"); }
...
fn trans_bench(disp: &str) {
    ...
    let mut best = f64::MAX;
    let mut sum = 0.0;
    for tr in d.tree.active_transitions.get_mut(&r.0).unwrap().values_mut() { tr.start_time_ms -= 1.0; tr.duration_ms = 1.0e12; }
    for _ in 0..100 {
        let t = std::time::Instant::now();
        d.tick_transitions();
        d.resolve_layout(VP.0, VP.1);
        let e = t.elapsed().as_secs_f64() * 1e6;
        best = best.min(e); sum += e;
    }
    eprintln!("[bench trans {disp}] best {best:.1}us mean {:.1}us computes/100={}", sum / 100.0, d.tree.taffy_computes - c0);
}
#[test] #[ignore] fn bench_trans_flex() { trans_bench("flex"); }
#[test] #[ignore] fn bench_trans_iflex() { trans_bench("inline-flex"); }
#[test] #[ignore] fn bench_trans_block() { trans_bench("block"); }
...
fn bench_hover_span_label() {
    ...
    let body = d.body();
    let list = d.create_element("div"); d.set_attribute(list, "class", "list");
    let mut rows = vec![];
    for i in 0..500 {
        let r = d.create_element("div"); d.set_attribute(r, "class", "row");
        let s = d.create_element("span");
        let tx = d.create_text(&format!("row {i} label")); d.append_child(s, tx); d.append_child(r, s);
        d.append_child(list, r); rows.push((r, tx));
    }
    d.append_child(body, list); settle(&mut d);
    let (r, tx) = rows[250];
    eprintln!("leaf? ifc_root={:?}", d.tree.get(tx.0).unwrap().ifc_root);
    let c0 = d.tree.taffy_computes; let mut best = f64::MAX;
    for k in 0..200 { d.set_attribute(r, "class", if k%2==0 {"row hot"} else {"row"}); let t = std::time::Instant::now(); d.resolve_layout(VP.0, VP.1); best = best.min(t.elapsed().as_secs_f64()*1e6); }
    eprintln!("[bench span-label hover] best {best:.1}us computes={}", d.tree.taffy_computes - c0);
}
