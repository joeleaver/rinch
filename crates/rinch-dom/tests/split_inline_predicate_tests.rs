...
fn the_field_is_recomputed_in_both_directions() {
    ...
    // `inline` <-> `block` changes no Taffy field. (`inline` <-> `inline-block`
    // is the newest aliasing pair, #592, and it is the one that made #597's
    // `is_inline_level` **crossing** test insufficient: both sides are
    // inline-level. `style_resolution/mod.rs`'s `ifc_dirty` trigger compares
    // the whole `DisplayMode` now, which covers `grid` <-> `inline-grid` too.)
