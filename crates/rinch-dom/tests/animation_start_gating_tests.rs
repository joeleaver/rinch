...
//! | Mutant | Killed by |
//! |---|---|
//! | **M1** `if self.tree.transitions_enabled` back around the animation block | `an_animation_declared_before_the_first_layout_runs_in_the_first_frame`, `the_first_frames_computed_style_carries_the_animated_value`, both `rinch` fixtures, and — through a failed precondition rather than their subject — `a_theme_change_keeps_a_running_animation_and_its_clock`, `a_theme_change_that_removes_the_declaration_stops_the_animation` and `a_spinner_hidden_and_shown_before_the_first_layout_restarts_from_zero` — 7 at `41c9119`; **39 at `bc451f0`** (1781 passed / 39 failed over `-p rinch-dom -p rinch`), the rise being all 8 `full_restyle_animation_refresh_tests` fixtures and 24 `transition_tests` animation fixtures whose two helpers stopped arming the flag by hand in round 3 |
