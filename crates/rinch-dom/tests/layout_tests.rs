...
/// [`rinch_dom::layout::parse_display_mode`] — the string path, which no
/// renderer calls.
///
/// It is `pub` and has **zero callers**: every real style resolution goes
/// through Stylo, and removing this function (with the rest of the parallel
/// dead Taffy-style builder in `layout.rs`) is tracked separately because it
/// removes public API. Until then its arms are a live promise, and the last two
/// inline-level values were added to it without a test — #595's `inline-flex`
/// and #607's `inline-grid`. This is that test, so deleting an arm fails
/// something.
///
/// `grid` and an unknown value are the controls: this function answers `None`
/// for both, so "every string maps to something" cannot pass, and the
/// `inline-grid` arm is visibly not the `grid` arm.
#[test]
fn parse_display_mode_answers_each_inline_level_spelling() {
    use rinch_dom::layout::parse_display_mode;
    use rinch_dom::node::DisplayMode;

    assert_eq!(
        parse_display_mode("display: inline-grid"),
        Some(DisplayMode::InlineGrid)
    );
    assert_eq!(
        parse_display_mode("display: inline-flex"),
        Some(DisplayMode::InlineFlex)
    );
    assert_eq!(
        parse_display_mode("display: inline-block"),
        Some(DisplayMode::InlineBlock)
    );
    assert_eq!(
        parse_display_mode("display: inline"),
        Some(DisplayMode::Inline)
    );
    assert_eq!(
        parse_display_mode("display: block"),
        Some(DisplayMode::Block)
    );
    assert_eq!(parse_display_mode("display: flex"), Some(DisplayMode::Flex));
    // Controls. `grid` has no arm either — a pre-existing gap this function
    // shares with `contents` and `none`, and deliberately not filled here:
    // answering `Block` for it would be a new claim about a dead function.
    assert_eq!(parse_display_mode("display: grid"), None);
    assert_eq!(parse_display_mode("display: ruby-text"), None);
    assert_eq!(parse_display_mode("width: 10px"), None);
}
