...
//!
//! Every stylo colour that becomes a `peniko::Color` comes through here — the
//! cascade (`color_from_stylo`), the bare-value parser (`layout::parse_color`)
//! and the keyframe extractor (`color_from_specified`) — so all three paths
...
pub(crate) fn color_from_absolute(color: &AbsoluteColor) -> Option<peniko::Color> {
