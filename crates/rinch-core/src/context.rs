...
/// windows: the app and its DevTools panel; two embedded `RinchContext`s) each
/// pump their own events through the same thread-locals, so without a marker
/// document B's `MouseMove` drives — and its `MouseUp` commits — document A's
/// drag.
///
/// This is **not** [`current_context_root`]. That is a store namespace, and
/// desktop shells deliberately leave it at the thread-global root `0` for every
...
/// drops (issue #139).
///
/// Push this once at the top of a runtime's event entry point. It **must** be a
/// guard, not a set/clear pair: dispatch runs arbitrary user code — handlers,
...
mod tests {
    ...
    // ── the dispatching-document marker (issue #139) ─────────────────────────

    /// The marker restores whatever it displaced, so nested dispatches — a
    /// document driven from inside another's handler — unwind correctly.
    #[test]
    fn the_dispatching_marker_nests_and_restores() {
        assert_eq!(current_dispatching_doc(), None, "nothing dispatching yet");

        let outer = push_dispatching_doc(7);
        assert_eq!(current_dispatching_doc(), Some(7));
        {
            let _inner = push_dispatching_doc(9);
            assert_eq!(current_dispatching_doc(), Some(9));
        }
        assert_eq!(current_dispatching_doc(), Some(7), "restored on drop");

        drop(outer);
        assert_eq!(current_dispatching_doc(), None);
    }

    /// A guard, not a set/clear pair: dispatch runs handlers, effect flushes and
    /// layout, any of which may unwind. A leaked marker would mis-attribute
    /// every later event on the thread.
    #[test]
    fn the_dispatching_marker_unwinds_with_a_panic() {
        let caught = std::panic::catch_unwind(|| {
            let _guard = push_dispatching_doc(5);
            panic!("a handler blew up mid-dispatch");
        });
        assert!(caught.is_err());
        assert_eq!(
            current_dispatching_doc(),
            None,
            "a panic in dispatch must not strand the marker"
        );
    }

    /// `doc_key` 0 is the "no document yet" sentinel — `RinchApp::doc_key`
    /// answers 0 before its document is assigned, and `next_doc_key` starts at 1.
    /// Pushing `Some(0)` would make every pre-mount dispatch look like one
    /// shared document (the mistake that made #134's first caret pass a silent
    /// no-op).
    #[test]
    fn doc_key_zero_pushes_no_document_at_all() {
        let _guard = push_dispatching_doc(0);
        assert_eq!(current_dispatching_doc(), None);
    }

    /// The marker is deliberately *not* the context root: desktop shells leave
    /// `scope_context_to_doc` off, so every window reports root 0 and the root
