...
//!
//! Manages reusable wgpu textures for uploading decoded frames (video,
//! render surfaces, etc.). Each layer gets its own texture slot so
//! multiple sources can be composited in the same render pass without
...
/// Manages wgpu textures for frame uploads.
///
/// Maintains a pool of textures, one per layer index.
/// Each slot's texture is reused across frames. If the resolution
...
impl FrameUploader {
    ...
    ///
    /// Each slot maintains its own texture so multiple layers can
