...
| `collab_sticky_index(pos) -> Option<Vec<u8>>` | A deep-link anchor for `pos`: a plain yrs `StickyIndex`, `encode_v1`, on the `text` of the textblock holding `pos` (`Assoc::After`; `Before` at a text's end; an empty text names the `Text` itself), so an app can resolve it with yrs alone. `None` when not collaborating, between blocks, for the starter paragraph of a zero-block CRDT, and while stalled or poisoned |
| `collab_resolve_sticky(&bytes) -> Option<Pos>` | Where such an anchor (from any replica) points now. Follows its character through every edit; a deleted character resolves to where it was; `None` once its **block** is deleted (a join counts: the projection has no move, so a split or join moves text as delete + insert) |
