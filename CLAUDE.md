...
**`prev_layout` is the box a node was last *painted* in, not the previous layout pass's box.** Layout never writes it; only the paint that consumes the region does (`NodeTree::consume_paint_dirty` — the software dirty-region frame, a full software repaint and `build_scene` all call it). Any number of resolves between two paints is therefore safe, which is what let the editor overlays (caret, selection, node outline) and the inset fast path (#280) stop forcing a full repaint per keystroke / per drag move: a second resolve in one frame used to overwrite the old rect and leave a ghost. A node whose *position* changed is also padded by its painted subtree's reach past its box (`opacity_layer_bounds` — shadow, outline, overflowing children) at the old and new rect; a subtree that walk cannot bound repaints in full. A removal records the removed box's pending old rect too. Still not covered: a **`transform`** change that leaves the layout alone adds only its new transformed rect (`repaint_old_rect_tests::a_transform_change_clears_the_old_painted_rect`, `#[ignore]`d). Pins: `crates/rinch/src/app/repaint_old_rect_tests.rs`.
