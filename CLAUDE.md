...
whichever document armed it, the same one or another. There is one pointer and
one slot, so a press that arms a drag while another is live means the old
drag's release was never delivered: swallowed in its own document (#189), or in
...
superseded one's `on_cancel` is superseded in turn. Two *simultaneous*
