...
`ime_set_preedit` (handle.rs) shows no composition, since its commit is refused; `attach` re-stamps the container on a re-mount. Desktop turns the **OS input method off** (`focus.rs`), greys **Cut and Paste** in the built-in context menu (`text_context_menu.rs`), keeps **Cut from becoming a quiet Copy** (`editor_cut`) and starts **no clipboard read** for a paste that cannot land (`dispatch_editor_paste` — a read is a 4s worst case and a permission prompt on some platforms). The web makes the capture `<textarea>` **`readonly`** so no soft keyboard rises (`sync_capture_read_only`), makes `cut` a no-op, and **keeps ownership of a printable key** while the capture textarea holds focus (`editor_input.rs`, `handle.insert_text(&key) || handle.is_read_only()`; the editor owns no key at all once focus leaves that textarea, #271) — that last one looks exactly like the check the rule forbids and is not: without it Space scrolls the page and a letter reaches the page's own shortcuts. `collab_receive` never goes through `commit` — a remote change must not be recorded back onto the CRDT — so a read-only editor keeps integrating peers' deltas **by construction**; never gate `integrate_remote`, and never toggle the flag around it. While read-only nothing is recorded or broadcast, which is why `load_doc`/`load_html` are refused on a **collaborating** read-only editor (a load with a session attached is a write to the shared document) and allowed otherwise (the app showing a document is not the user editing one). The flag lives on the handle, not the view: it works before mount and survives a re-mount; the container carries `data-pm-readonly="true"` while it is on.

**Caret affinity at a soft wrap is view state beside the selection** (#301). The end of one visual line and the start of the next are one model `Pos`, so `EditorCore` keeps `caret_hint: Option<(Selection, CaretAffinity)>` (`rinch_core::dom::CaretAffinity`, `Downstream` by default — the lower line's start — or `Upstream`, the upper line's end). It applies **only while the selection equals the stored one**, so every other writer (typing, undo, a load) retires it without knowing it exists, and a plain `set_selection` clears it explicitly, even to the same caret. A **document** change — a local `commit` or a remote integrate — carries it to the new selection when it only *shifted* it (`EditorCore::carry_caret_hint`: both ends moved by one amount, the head at the same offset in a textblock of the same type and attrs whose content **up to the head** is unchanged), as CodeMirror maps `assoc`: a peer typing in an earlier paragraph, or after the caret in its own, leaves an upstream caret at the end of its line; an edit before the caret in its own block drops it, and so does a commit landing any selection other than the hinted one (no revival by another route). Right before a `hard_break` the caret is always `Upstream` (`EditorCore::affinity_at`): the break has no bytes in the host text, so the positions before and after it share one offset, and the model says which line each belongs to. The test is positional, not the transaction's mapping, because a remote integration rebuilds the document and has none. `EditorHandle::set_selection_with_affinity` stores it **before** the dispatch (so `on_selection_change` — where a popup reads `caret_rect(head)` — already sees it; inert unless that selection lands) and owes an overlay pass when only the side changed; `caret_affinity()` / `caret_affinity_at(pos)` read it; `caret_rect` at the head and the caret overlay draw with it, through `DomDocument::query_caret_rect_with_affinity` / `query_caret_position_with_affinity` (trait defaults: the affinity-blind answer, so the mock keeps compiling). rinch-dom hands it to `parley::Cursor`; the web draws `Upstream` as the collapsed range (Chrome draws a wrap point at the upper line's end) and `Downstream` at the start edge of the character after it (its right edge in a right-to-left run). The affinity-blind `query_caret_rect` / `query_caret_position` answer `Downstream` on both backends. Setters, identical on both backends: End sets `Upstream` when its edge is a soft wrap point (the line's end, after any hanging space — desktop snaps a hit before the space with `text_query::hanging_whitespace_end`); a press and an Up/Down hit set `Upstream` when the upstream caret is nearer the hit's y than the downstream one (`editor_hit_affinity` / web `hit_affinity`; nearness, not containment — a caret rect covers the glyphs, not the line box, and Up/Down probes 1.5 caret heights down, which on a short-caret face lands in the leading: CI's did); Home and the rest are `Downstream`. The visual-line motions measure from the head's affinity-aware rect, so Home/End, the web's soft-line deletes and Up/Down follow it. Desktop's End no longer steps back one position (on an `overflow-wrap` glyph wrap that stopped before the line's last letter). Not the `<input>`/`<textarea>` engine (#941), nor the selection-rect slivers (#1010). Pins: `rinch/src/app/editor_caret_affinity_tests.rs`, `rinch-web/tests/editor_soft_line.rs` (+ `_review.rs`, Chrome 153's answers), `handle.rs` `tests::caret_affinity`.

**Scrolling the caret into view is decided on the handle, from what changed the state — never from the caret's geometry** (#837). `EditorCore` carries a `ScrollGate` (ProseMirror's `tr.scrollIntoView()`): `commit` arms it for a **local** change that altered the selection or the document, and `EditorHandle::update_caret` fulfils it — a `NodeHandle::scroll_into_view()` on the view's `scroll_anchor()` (the caret, a node outline, a cell selection's head-cell wash) — on the first caret pass that **has** an anchor, so an Enter into a not-yet-measured paragraph still scrolls once it is laid out. It is **not** armed by focus (#846's review): a press that places the caret arms through `set_selection` anyway, and one that does not (a task checkbox, a right-click on an image) must not jump to the old caret — nor does `EditorHandle::focus()`, which arms nothing (giving the keyboard back after a dialog must not jump either); an app that wants something on screen asks with `scroll_into_view`. **But focus performs a scroll already owed**: only the focused editor's caret pass fulfils the gate, so a `set_selection` of a caret on a blurred editor stays armed until the next `focus()` (#922's review, D4 — the deep-link `set_selection(caret); focus()` relies on it; pinned by `focus_performs_a_caret_scroll_already_owed_and_arms_none`). It is **not** armed by `collab_receive` (no `commit`), `load_doc`/`load_html`, a read-only refusal (the arm sits after `refuses`), or a public `update(..)` transaction that does not call `tr.set_selection` — that one is the shape of an app's or a peer's edit elsewhere, which maps the caret. The handle's own input paths (`insert_text`, `set_selection`, paste, `ime_delete_surrounding`) go through the private `dispatch(.., input: true)` and arm whenever the selection or document changed. The view's `ViewRequest::ScrollSelectionIntoView` still says "the overlay moved" and the handle reads it for nothing else: #837's first cut gated on it, and its review measured it pulling a user who had scrolled away back to the caret on a resize reflow, a remote edit above the caret, and every scroll of a virtualized editor (whose blocks move as they are measured). A non-empty text range drops an owed scroll (a Shift+arrow head is not revealed). Desktop applies the scroll after layout and moves only the **nearest** scroll container; the web calls `scrollIntoView({block: "nearest", inline: "nearest"})` at once, which moves every scrollable ancestor (**#842**) — and that makes `NodeHandle::scroll_into_view` live on `rinch-web` for any caller, where it used to be the trait's no-op. A caret moved into a virtualized block that was never laid out never scrolls (**#845**). Pins: the handle's `ScrollGate` tests, `rinch/src/app/editor_scroll_gate_tests.rs`, and `rinch-web/tests/editor_scroll_into_view.rs` (run it `--features collaboration` too). Guide: `docs/src/guide/contenteditable.md#keeping-the-caret-in-view`.

**Programmatic focus and reveal: `EditorHandle::focus`, `EditorHandle::scroll_into_view`** (a followed deep link: `set_selection(range)`, `scroll_into_view(from, to)`, `focus()`; guide: `contenteditable.md#focus-and-scrolling-from-code`). `focus()` asks the platform through `registry::set_focus_handler`: the web registers one (`editor_input::focus_editor`, the focusing half of `handle_mousedown` — focused editor, capture textarea focused with `preventScroll`, `refresh_caret`); desktop registers none, so the handle calls `NodeHandle::focus` on its container, which posts the ordinary focus request, and `RinchApp::try_focus_input` maps a registered editor container to `FocusTarget::Editor` (so `NodeHandle::focus` on a container works on desktop too). `scroll_into_view` stores the range on `EditorCore::reveal` (carried through local commits by `carry_reveal`, dropped by a load, replaced by a later call), marks the document in `registry::reveal_owed` and runs `request_overlay_refresh`; `update_all_carets` then calls `EditorHandle::reveal_pass` for **every** editor, focused or not, after the caret pass, which places two hidden `data-pm-reveal` probes (`RinchDomEditorView::position_reveal`: 1px wide, the caret line grown by `REVEAL_MARGIN` = 16px, clamped to the container top and the last block's bottom) over the end and then the start and calls `NodeHandle::scroll_into_view` on them in that order — two `nearest` scrolls leave the start visible whichever side the range was on, which one box around a range taller than the scroller would not. The reveal waits on the handle until a pass has geometry for its **start**; the end is revealed with it if it has geometry too, and otherwise the start alone (#922's review, D3: a virtualized range whose end block stayed collapsed used to stay pending for good). A pending reveal is dropped after `REVEAL_PATIENCE` (8) consecutive passes with no geometry for its start (`reveal_misses`, reset per request); a `display: none` editor answers zero-size geometry on both backends, so its reveal is consumed at once and scrolls nothing. Desktop folds `registry::overlay_pass_owed` (`reveal_owed` is its old name, kept as an alias) into `has_dirty_nodes`/`has_pending_layout` and the resolve short-circuit, so a request from an effect or timer wakes one resolve; the pass clears the flag fulfilled or not, so an unfulfillable reveal never spins frames. **A selection change owes the same pass** (#1001): `EditorCore::commit` calls `registry::owe_overlay_pass` whenever the selection moved, because a selection-only transaction dirties no DOM and a `set_selection` / `command("selectAll")` from a toolbar, timer or effect otherwise drew no caret or highlight until something else laid out. The input paths run `refresh_editor_overlays` right after their edit, which clears it in the same turn. The web has no frame loop to ask, so it registers `registry::set_overlay_pass_scheduler`: the first owe queues a microtask that runs `refresh_caret` only if a pass is still owed (`any_overlay_pass_owed`) — a keystroke pays one empty microtask. Pins: `rinch/src/app/editor_programmatic_selection_tests.rs`, `rinch-web/tests/editor_programmatic_selection.rs`, `handle.rs` `tests::reveal::a_selection_change_wakes_*`. Virtualization's `pre_layout` materializes, and `protected_block` keeps laid out, the blocks holding a pending reveal's start **and end** (`pending_reveal_range`). Both calls are no-ops before mount. `scroll_into_view_aligned(from, to, ScrollAlign::Fraction(f))` rides the same `reveal` (it carries the `ScrollAlign`; `scroll_into_view` is `Nearest`) but places **one** probe, the start's caret line with no margin, and calls `NodeHandle::scroll_to_fraction(f, REVEAL_MARGIN)` on it: `DomDocument::request_scroll_to_fraction` (default: a plain `request_scroll_into_view`), queued on desktop in `NodeTree::scroll_to_fraction_requests` and applied by `apply_scroll_into_view` after the nearest ones (top at `clamp(f*H, margin, H - margin - h)` below the inside of the nearest scroll container's top border, `H` its **padding** box — the web's `clientTop`/`clientHeight` — scroll clamped to `0..=max`), and on the web applied at once (`scrollTop` of the nearest `overflow-y: auto|scroll` ancestor, else the page's scrolling element, then a `nearest` `scrollIntoView` for the scrollers further out). In a virtualized desktop editor the start can land a fraction of a line off (the blocks entering the window settle from estimated to measured heights after the scroll). Pins: `handle.rs` `tests::reveal`, `rinch/src/app/editor_focus_and_reveal_tests.rs`, `rinch-web/tests/editor_focus_and_reveal.rs`.

Command names (dispatch by string): `toggleBold/Italic/Underline/Strike/Code/Highlight/Subscript/Superscript`, `setParagraph`, `setHeading1..6`, `setCodeBlock`, `setTextAlign{Left,Center,Right,Justify}`, `toggleBulletList`, `toggleOrderedList`, `toggleTaskList`, `wrapInBlockquote`, `sinkListItem`/`liftListItem` (indent/outdent), `insertHorizontalRule`, `insertHardBreak`, `insertTable`, `addRow{After,Before}`, `addColumn{After,Before}`, `deleteRow`/`deleteColumn`/`deleteTable`, `mergeCells`/`splitCell`, `removeLink`, `deleteSelection`, `deleteChar{Backward,Forward}`, `deleteWord{Backward,Forward}` (to the word motion's boundary; at a textblock edge, or with a selection or an inline atom beside the caret, the char delete — bound to Ctrl+Backspace/Delete, Alt on macOS, and to no key on wasm, where the browser's `beforeinput` names them; #303 — there are no line-delete commands: on the web `deleteSoftLine*` goes to the visual line's edge and `deleteHardLine*` to the textblock's, #301), `undo`/`redo`. (Adding a link needs an `href` arg, so it is **not** a string command — use `handle.toggle_link(href)`.)

**Inherited marks follow ProseMirror's `inclusive`.** `MarkSpec::inclusive` (default `true`; `MarkSpecBuilder::inclusive(false)`) decides whether a mark carries on past its end. `ResolvedPos::marks` — which `marks_at`, `Transaction::insert_text` (so typing, IME commit and every other text-entry path), `add_stored_mark` and `is_mark_active` all read — gives the run's own marks inside a run, the marks of the run *before* a boundary, and the run after at a textblock's start; a non-inclusive mark is dropped at a boundary unless the run after carries a mark of the same type (ProseMirror asks for the same attrs; see the seam note below), and at a textblock's start. The starter kit's `link` is non-inclusive (every other mark is inclusive), so text typed right after a link is plain, and a caret at a link's start or end reports no link — except at the seam of two different links, where both continue the first (below). The collab projection resyncs a non-inclusive mark **per char**, as it does `@atom` (below): yrs puts an insert at a range's end inside the range, and a per-span clear-and-rewrite of the link would revert a peer's concurrent change to it. A char typed right after one is **inserted with its attributes** (`splice_min_with_marks`, yrs `insert_with_attributes`) when it carries exactly what the char after it carries (`inserts_after_non_inclusive`): the insert then walks past every formatting marker between the two and writes none, so a peer's concurrent extension of the link, or link over the following text, survives (review of #901, D2 — insert-then-clear's `format(X, link: null)` deleted the end marker those depended on). **Any other shape takes insert-then-clear**, because there `insert_with_attributes` stops at the first mismatching marker and writes `{link:null} X {link:old}` — and that trailing restore brings a peer's concurrently *removed* link back over the following text (round 2, F1/F2: a bold link; an image after the link). **The yrs API cannot express "plain X between two different links" without such a restore** (every write that changes a key's value writes a restore after it unless the next item already matches, and `format` deletes every same-key marker up to the next char), so `ResolvedPos::marks` departs from ProseMirror at that seam: typed text continues the first link and needs no marker (round 2, F3/F4). **Known limitation, pinned (#923):** a peer's concurrent href change or adjacent link can take the typed char in (an href change in one client-id order, a link over the following text in both) — nobody's edit lost, convergent. Not for atoms: a literal U+FFFC absorbed that way reads back as a second image. Pins: `pos::resolved` tests, `typing_*` in `state/transaction.rs`, `a_caret_right_after_a_link_*` in `handle.rs`, and `typing_after_a_link_*` / `a_char_typed_after_a_link_reaches_the_peer_unlinked` in `rinch-editor-collab/tests/collab.rs`.

**Markdown shortcuts (input rules).** As you type, the default `MarkdownInputRulesPlugin` rewrites markdown shortcuts (these run inside `EditorHandle::insert_text`, so every text-entry path gets them). Block shortcuts at the line start: `# `…`###### ` → headings, ` ``` ` → code block, `---`/`***` → horizontal rule (fires on the third character, only when the marker is a paragraph's **whole** content — typed in front of existing text it stays text; the caret lands in the next block of the same parent, or a fresh empty paragraph appended inside it), `> ` → blockquote, `- `/`* `/`+ ` → bullet list, `1. ` → ordered list, `[ ] `/`[x] ` → task list. Inline mark shortcuts (fire on the closing delimiter): `**bold**`/`__bold__`, `*italic*`/`_italic_`, `~~strike~~`, `==highlight==`, `` `code` ``. The rule set lives in `rinch-editor-core/src/input_rules.rs` (`markdown_input_rules()`); add a `mark_input_rule`/`wrapping_input_rule` there to extend it. Task items render a checkbox from their `checked` attr via the default stylesheet (`[data-pm-type="task_item"]`); `Enter` makes a fresh unchecked item, `Enter` on an empty item exits the list.

**Inline decorations** (#836). A plugin's `decorations()` may return `Decoration::inline(from, to, attrs)` as well as widgets: the view splits the covered text into `<span data-pm-deco class=…>` segments (an undecorated run stays a bare text node) and re-syncs them on every transaction. The core does **not** remap a decoration set — `EditorState::decorations()` asks every plugin afresh, so a plugin that caches ranges maps them itself through `tr.mapping()` in `Plugin::apply`. The stylesheet ships `pm-spell-error` / `pm-grammar-error` as wavy underlines, which rinch-dom paints natively (`text-decoration-style: wavy`; the other styles parse and paint solid). Moving the caret re-runs decorations, so a decoration can vanish under a press; the desktop right-press path therefore resolves the press once, on the tree as pressed, before anything moves the caret.
...
| `crates/rinch-editor-view/src/keys.rs` | `EditorKey` — a key press offered to `EditorHandle::on_key` |
...
**Staged scope (design A22):** the first milestone covers **flat text-blocks + marks** (`paragraph`/`heading`/`code_block` with text + bold/italic/link/… marks), the **list containers** `bullet_list`/`ordered_list`/`list_item` (nested into each other and around text-blocks to any depth), the **leaf block atom** `horizontal_rule` (a block with no content of its own, projected as a block with empty text; since #836) and the **inline atoms** (`image`/`hard_break` — one U+FFFC char of the block's text carrying a reserved `@atom` formatting attribute that names the node type and its attrs). Anything else — `blockquote`, tables, `task_list`/`task_item` — is `CollabError::Unsupported`: the adapter **fails loud** rather than silently dropping a change (a silent drop would reintroduce the exact divergence class the editor rewrite killed).

**`horizontal_rule` in scope is a wire change that needs a coordinated upgrade.** The format tag did not move (a rule needs no wire shape of its own), so a peer built before #836 accepts the bytes and then refuses the rule on read — `Unsupported`, which **poisons its session** (#196). Loud, not silent, but every peer sharing a document (PlotWeb's included) must be on a build with leaf atoms in scope before any of them inserts a scene break.
...
| `collab_sticky_index(pos) -> Option<Vec<u8>>` | A deep-link anchor for `pos`: a plain yrs `StickyIndex`, `encode_v1`, on the `text` of the textblock holding `pos` (`Assoc::After`; `Before` at a text's end; an empty text names the `Text` itself), so an app can resolve it with yrs alone, **on a `Doc` built with `Options { offset_kind: OffsetKind::Utf16, .. }`** (a default `Doc::new()` answers `get_offset` in neither unit). `None` when not collaborating, between blocks, for the starter paragraph of a zero-block CRDT, and while stalled or poisoned |
| `collab_resolve_sticky(&bytes) -> Option<Pos>` | Where such an anchor (from any replica) points now. Follows its character through every edit; a deleted character resolves to where it was; `None` once its **block** is deleted (a join counts: the projection has no move, so a split or join moves text as delete + insert). At the top level the projection matches untouched blocks by node identity (#917), so Enter at a paragraph's start moves its anchors into the new empty paragraph, and deleting an empty line above it or toggling a list on it gives `None`; nested blocks compare structurally and keep them |
...
| `crates/rinch-editor-collab/src/sticky.rs` | `CollabDoc::sticky_index`/`resolve_sticky` (and the `CollabSession` pair): model `Pos` ↔ plain yrs `StickyIndex` bytes, mapped by the projection's index-for-index mirror of the model tree, checked per level (node type) and at the block (its text) |
...
| `crates/rinch-editor-view/src/handle.rs` | `EditorHandle`'s collab methods (`start_collaboration_host/guest`, `collab_receive`, `collab_state_vector`, `collab_sync_diff`, `collab_snapshot`, `collab_take_error`, `collab_outbound_stall`, `collab_sticky_index`, `collab_resolve_sticky`, `stop_collaboration`, `is_collaborating`, `is_collaboration_poisoned`) — **not** in `rinch-editor-collab` |
...
**A dirty region clears what the last paint drew, not what the tree says now.** Each node keeps what it was last *painted* with — `prev_layout` (its box) and `Node::painted` (its own `box-shadow`/`outline`/`text-shadow` reach — the last is one and a half blur radii (three standard deviations, where the blurred mask of `paint/text_shadow.rs` stops) around its offset, measured from the border box, so text that overflows its box casts past it; an IFC root's reach also holds the `text-shadow` lists its inline elements declare, since it draws their text (#1048; not in a `text-overflow: ellipsis` line, where every run casts the root's list — #1065) — its own transform, and whether it clipped and how it was positioned — #909) — all **parent-relative**, and only the paint that consumes the region writes them (`NodeTree::consume_paint_dirty`, O(1) per dirty node — O(text runs) for an IFC root, and a box-less inline's consumption re-reads its root's ink; the software dirty-region frame, a full software repaint and `build_scene` all call it). `paint::previous_painted_rect` sums them up the box-tree chain, so a node's old rect is exact however many resolves ran since, when an *ancestor* moved rather than the node, and when its ink or transform changed in the same frame. `compute_dirty_region` takes, per dirty node, that painted rect and its current box grown by its current own ink (a box-less inline names its IFC root's rect, grown by that root's ink, where it is and where it was painted); a **positioned** box (`absolute`, `fixed`, `relative`) whose position changed is also grown by its subtree's reach (`opacity_layer_bounds`), which is what carries an untouched overflowing child along with a dragged panel or a reflow-shifted `relative` row (a subtree that walk cannot bound repaints in full). Static boxes are not walked — a reflow shifts every row — so a static box's in-flow child whose own ink reaches past a reflow-shifted parent is not covered (#880's probe J2, also on main); and the region stops being measured once it reaches `paint::FULL_REPAINT_FRACTION` of the surface. A removal records each removed node's painted rect, children before the parent's state is forgotten. This is what let the editor overlays (caret, selection, node outline) and the inset fast path (#280) stop forcing a full repaint per keystroke and per drag move: a second resolve used to overwrite the old rect, and the forced full repaint also hid a move and an ink change landing in one frame (#880's review). An IFC's direct write of an atomic inline's position pushes the box paint-dirty when it moves, so a chip moved by the text before it is repainted, overflowing children and all. `build_scene` also sets `has_previous_frame = false`, so a software frame after a GPU one is full. The paint prune (`paint_node`, `paints_nothing_without_visit`) tests each box grown by the same own ink, so damage over a shadow's or an outline's bleed and not its box repaints it there (#889 item 1) — except an atomic inline's (`inline-block`/`-flex`/`-grid`): it is drawn by its IFC root, which is culled by its own box, so damage over only an inline-block's shadow bleed below its line is still not repainted (pre-existing; #889). Not covered: the `text-shadow` of text that overflows its box, a `filter` reach, and the #550 partial escape of an absolute from an `overflow` box inside a moved subtree. Pins: `crates/rinch/src/app/repaint_old_rect_tests.rs` (and `build_scene_consume_tests.rs`, `--features embed`).

**Damage is clipped by what clips the damaged box** (#909). Every rect `compute_damage` names for a node — where it paints now, where it was painted, its IFC root's or owner's box — is intersected with that node's clip chain (`paint::clip_chain_bounds`) before it joins the region, and a removed node's rect is intersected when it is recorded; so a row a scroller moved below its viewport names nothing, and a keyed `for` change in a 300x400 scroller repaints 304 x 404, not the column down to the window's edge. The walk mirrors what clips a box in paint and errs wide in three places: a `position: fixed` box takes no clip above it, an `absolute` box skips every clipper below its containing block (whose own clip it takes, as `Collector::span` does), and a `display: contents` or degenerate clipper is not taken. The **current** chain answers for where a box paints now — it is exactly what the next paint clips with. The **painted** chain answers for its old pixels from each node's `PaintedState` (`clips`, `position`, transform) and `prev_layout`, walked along the **current** parents. Two things make that true. Every style change pushes its node, so a painted state holds between two consumptions; and the paint consuming a whole-document restyle, which pushes nothing, re-reads **every** painted node's state (`NodeTree::consume_paint_dirty` — O(n) on a frame that has just restyled and repainted all n, and it also stops ink and transform going stale after a theme change). And a move to another parent (`append_child`, `insert_before`, `insert_child`, `replace_node`'s incoming node) records the subtree's old rects under the old chain and forgets its painted state first (`record_pixels_left_by_move`), exactly as a removal does, so a moved node is not placed along parents it was not painted under; a reorder within one parent and a never-painted or parentless node pay nothing. A node never painted gives no clip. **Not covered, and pre-existing:** three detaches keep the painted state of what they take out — `set_text_content`'s orphans (`parent = None`, painted state kept), `replace_node`'s displaced node, and `set_inner_html`'s freed children, whose rects are not recorded — so such a node attached again has its old rect placed along its new parents (#966); and `Frame::Painted` places a box by its *current* `position`, so a same-frame static↔fixed change (or a clipper becoming the containing block) leaves old pixels (#961). A rect clipped away entirely still counts as *named*, so the `boxed_owner` fallback does not repaint the clipper's box in its place. The contents-wrapper subtree rects are not clipped (one chain cannot speak for a subtree whose boxes may escape it). Pins: `crates/rinch/src/app/damage_clip_chain_tests.rs`.
...
span is its IFC content with a `0x0` box. A second route — an inline flex or
grid item behind a `display: contents` wrapper, which rinch did not blockify —
was reachable from ordinary markup until #998 closed it; no route but
construction in a fixture is known now. A future component that puts `overflow: hidden` on an inline-level
...
**Cost.** `TinySkiaPainter::push_clip` fills and intersects a mask over the
clip's bounds (it allocated a full-surface `Mask` per push until the painter
pooled them — see **The software painter's caches** above), so a
...
**Execution order is a contract** (#154): effects observing the same signal run in **registration order** (the order their `Effect`/`Memo` was created), and the pending queue drains FIFO — so an effect registered *after* an `rsx!` tree sees the post-patch DOM in the same flush ("run me last"), and a signal written from inside an effect queues its observers *behind* the current flush rather than preempting it. **The one exception is the running effect itself** (#343): a write flushed while its own body is on the stack — any write made with no batch open, and every flush runs with the batch closed — skips that effect rather than queueing it, so a cleanup of a scope the body disposes cannot re-run the disposing effect (that is what keeps `for_each_dom`'s reconcile from re-running for a row's cleanup write). A body running inside an **open** batch (`Effect::new`/`Effect::run` in a handler) queues its writes, and does re-run for them. Deliberate, not a bug: queued, a body that disposes a fresh scope each run would never settle. Pinned by `reactive::effect::self_wake_tests`. Enforced by `BTreeSet<ObserverId>` subscriber sets (ids are monotonic and never reused, so ascending id *is* registration order) plus `pop_front` in `flush_effects`. Don't swap either for a `HashSet`/LIFO. See `docs/src/guide/reactivity.md#execution-order`.
...
**The echo is not written back** (issue #238, part 2). `TextInput`, `PasswordInput`, `Textarea` and `NumberInput` share one `value_fn` binding, `rinch_components::value_binding::bind_value_fn`, which writes only a value the field does not already show — read through `NodeHandle::live_value` / `DomDocument::live_value`, the text the field holds *now*. That is the `value` attribute on desktop (the runtime mirrors each edit into it before dispatching `oninput`, so the trait default is honest there) and the `.value` **property** of an `<input>`/`<textarea>`/`<select>` on web, where the attribute keeps the last programmatic write while the user types. **Never compare against `get_attribute("value")` to ask what a field says** — on web that is a fossil, and a guard reading it skips a change back to the fossil's own value. `ColorPicker`/`ColorInput` read `live_value` too; the per-component `typed` record they kept for this (#235) is gone. `MockDomDocument::__type_into` moves a mock control's live text without its attribute, which is how a host test reaches the web's state.

**Undo/redo on a focused field** (issue #288): Ctrl+Z, and Ctrl+Shift+Z / Ctrl+Y (Cmd on macOS; never with Alt — AltGr on Windows — and refused while an IME preedit is shown), reach `EditableState`'s stack through `handle_input_edit_command`, so `oninput` fires with the restored text and `readonly`/`disabled` refuse them like any edit. One key press is one step: `EditableState::execute` folds a command's operations into a `TextOperation::Group` (typing over a selection), and `handle_input_edit_command` folds the rewrite a keystroke's own `oninput` made into that keystroke's step (`adopt_focused_input_rewrite_from_dom`, `UndoStack::mark` / `EditableState::group_undo_since`) — or drops the step outright when the rewrite put the text back as it was (a rejected keystroke). **Every other adopted `value` write clears the undo and redo stacks** (`adopt_focused_input_value_from_dom`), as Chrome does for any script write to `.value`: a clear after submit is not undoable. (A keystroke's own `oninput` rewrite is the one divergence: Chrome clears history for it too, so a normalizing field has no undo on the web.) No typing coalescing, and the text context menu has no Undo row. Pins: `input_commit_tests` (`ctrl_z_*`, `a_normalizing_field_*`, `a_programmatic_write_clears_*`, `a_rejected_keystroke_*`, …).
...
- **Components sitting side by side where you expected a column**: `display: inline-flex` is **inline-level** (#595) — an *atomic inline*, like `inline-block`: it joins the line around it and shrink-wraps, and only its inside is a flex container. `Button`, `Badge`, `Checkbox`, `Switch`, `Avatar`, `ActionIcon`, `CloseButton`, `Loader`, `Pagination` and `Center` all declare it, so two of them in a plain `<div>` share a line, exactly as in a browser and as on rinch-web. Put them in a `Stack` (or any `display: flex` parent) to stack them — CSS blockifies a flex item, so the `inline-flex` is `flex` there and nothing about this applies. That includes an item behind any number of `display: contents` wrappers — every `for` row, `if` branch and component site `rsx!` emits — because blockification reads the **layout** parent, the nearest ancestor that is not `display: contents` (#998; rinch passed the DOM parent until then, so a wrapped `span` in a `Group` stayed `inline`). `display: inline-grid` is inline-level too, and has been since #607 — an atomic inline whose *inside* is a grid container rather than a flex one. `display: inline-block` is the third, and its **inside is a block container** (#592): its children stack, its inline runs get anonymous block boxes, and its own `font-size`/`font-weight`/`line-height` size its box. It laid its children out in a row until #592, because the Taffy container it built was a flex one
