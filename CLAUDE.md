...
| **Query (read state)** | `can_run(name)`, `is_mark_active(mark)`, `active_link_href() -> Option<String>` (a range: its first link; a caret: only strictly inside a link), `current_block_type()`, `in_node_type(type)`, `doc() -> Node`, `state()`, `selection()` |
...
**Inherited marks follow ProseMirror's `inclusive`.** `MarkSpec::inclusive` (default `true`; `MarkSpecBuilder::inclusive(false)`) decides whether a mark carries on past its end. `ResolvedPos::marks` — which `marks_at`, `Transaction::insert_text` (so typing, IME commit and every other text-entry path), `add_stored_mark` and `is_mark_active` all read — gives the run's own marks inside a run, the marks of the run *before* a boundary, and the run after at a textblock's start; a non-inclusive mark is dropped at a boundary unless the run after carries the same mark with the same attrs, and at a textblock's start. The starter kit's `link` is non-inclusive (every other mark is inclusive), so text typed right after a link is plain, and a caret at a link's start or end reports no link. The collab projection resyncs a non-inclusive mark **per char**, as it does `@atom` (below): yrs puts an insert at a range's end inside the range, and a per-span clear-and-rewrite of the link would revert a peer's concurrent change to it. A char typed right after one is also **inserted with its attributes** (`splice_min_with_marks`, yrs `insert_with_attributes`), not inserted and then cleared: the clear `format(X, link: null)` deleted the link's own end marker, which a peer's concurrent extension of the link, or link over the following text, depended on — so that edit was lost (review of #901, D2). The insert steps past the end marker and writes none of its own. **Known limitation, pinned:** a peer's concurrent href change or adjacent link can then take the typed char in (an href change in one client-id order, a link over the following text in both) — nobody's edit lost, convergent, the char linked where it was typed plain. Not for atoms: a literal U+FFFC absorbed that way reads back as a second image. Pins: `pos::resolved` tests, `typing_*` in `state/transaction.rs`, `a_caret_right_after_a_link_*` in `handle.rs`, and `typing_after_a_link_*` / `a_char_typed_after_a_link_reaches_the_peer_unlinked` in `rinch-editor-collab/tests/collab.rs`.
