...
- **A heading suddenly got bigger and bold**: since #627 the UA stylesheet gives `<h1>`–`<h6>` the browser's typography (`2em`…`0.67em`, `font-weight: bold`, `em` block margins) and `<th>` `font-weight: bold; text-align: center`, so a bare heading renders as a heading on desktop the way it always did on rinch-web. They are cascade rules, so any author `font-size`/`font-weight`/`margin` still wins — which is why `Title`, `Modal`, `Drawer` and the editor's own stylesheet are unmoved. `<th>` gets no `display`: rinch has no table formatting context, so `table` stays `display: block` and the cells stay `inline`
