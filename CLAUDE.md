...
**Undo/redo on a focused field** (issue #288): Ctrl+Z, and Ctrl+Shift+Z / Ctrl+Y (Cmd on macOS), reach `EditableState`'s stack through `handle_input_edit_command`, so `oninput` fires with the restored text and `readonly`/`disabled` refuse them like any edit. One key press is one step: `EditableState::execute` folds a command's operations into a `TextOperation::Group` (typing over a selection), `adopt_text` makes a rewrite one step, and `handle_input_edit_command` folds the rewrite a keystroke's own `oninput` made into that keystroke's step (`UndoStack::mark` / `EditableState::group_undo_since`) — without that, undo restored the raw text, the normalizer rewrote it (a fresh edit, clearing redo) and the user could never undo past it. No typing coalescing, and the text context menu has no Undo row. Pins: `input_commit_tests` (`ctrl_z_*`, `a_normalizing_field_*`, …).
